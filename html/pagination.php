...
function pagination_list_footer($list)
{
	...
	if (isset($list['pageslinks'])) { $html .= $list['pageslinks']."\n"; }
	$prefix = '';
	if (isset($list['prefix'])) { $prefix = $list['prefix']; }
	$limitstart = '';
	if (isset($list['limitstart'])) { $limitstart = $list['limitstart']; }
	$html .= '<input type="hidden" name="'.$prefix.'limitstart" value="'.$limitstart.'" />'."\n";
	$html .= '</div>'."\n";
...
function pagination_list_render($list)
{
	...
	if (isset($list['pages'])) {
		foreach ($list['pages'] as $k => $page) {
			if (!isset($list['active']) || !$page['active']) {
				$currentPage = $k;
			}
		}
	}
	...
	if (isset($list['start']) && isset($list['start']['data'])) {
		$html .= $list['start']['data'];
	}
	if (isset($list['previous']) && isset($list['previous']['data'])) {
	...
	if (isset($list['next']) && isset($list['next']['data'])) {
		$html .= $list['next']['data'];
	}
	if (isset($list['end']) && isset($list['end']['data'])) {
		$html .= $list['end']['data'];
	}
